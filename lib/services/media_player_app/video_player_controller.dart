...
class VideoPlayerController {
  ...
  Future<void> play() async { try { await _method.invokeMethod('play'); } catch (_) {} }
  Future<void> pause() async { try { await _method.invokeMethod('pause'); } catch (_) {} }
  Future<void> togglePlayPause() async { _state.isPlaying ? await pause() : await play(); }
  Future<void> seekTo(Duration position) async { try { await _method.invokeMethod('seekTo', {'position': position.inMilliseconds}); } catch (_) {} }
  Future<void> seekBy(Duration delta) async {
    final target = _state.position + delta;
    final clamped = Duration(milliseconds: target.inMilliseconds.clamp(0, _state.duration.inMilliseconds));
    await seekTo(clamped);
  }
  Future<void> setSpeed(double speed) async { try { await _method.invokeMethod('setSpeed', {'speed': speed}); } catch (_) {} }
  Future<void> setVolume(double volume) async { try { await _method.invokeMethod('setVolume', {'volume': volume.clamp(0.0, 1.0)}); } catch (_) {} }
  Future<void> setBrightness(double brightness) async { try { await _method.invokeMethod('setBrightness', {'brightness': brightness.clamp(0.0, 1.0)}); } catch (_) {} }
  ...
  Future<void> disableSubtitles() async { try { await _method.invokeMethod('disableSubtitles'); } catch (_) {} }
  
  // New Methods for Custom Overlays
  Future<void> setSubtitleDelay(int delayMs) async { try { await _method.invokeMethod('setSubtitleDelay', {'delayMs': delayMs}); } catch (_) {} }
  Future<void> setAudioDelay(int delayMs) async { try { await _method.invokeMethod('setAudioDelay', {'delayMs': delayMs}); } catch (_) {} }
  Future<void> setEqualizer(List<double> bands) async { try { await _method.invokeMethod('setEqualizer', {'bands': bands}); } catch (_) {} }
  
  Future<void> setAspectRatio(int mode) async { try { await _method.invokeMethod('setAspectRatio', {'mode': mode}); } catch (_) {} }
  Future<void> enterPiP() async { try { await _method.invokeMethod('enterPiP'); } catch (_) {} }
